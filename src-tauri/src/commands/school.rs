...
#[tauri::command]
pub async fn get_all_schools_of_thought(
    state: State<'_, EncyclopediaDb>,
) -> Result<Vec<SchoolOfThought>, String> {
    let rows = state
        .list_entities(Some(EntityType::SchoolOfThought))
        .await
...
#[tauri::command]
pub async fn get_school_of_thought(
    state: State<'_, EncyclopediaDb>,
    name: String,
) -> Result<Option<SchoolOfThought>, String> {
    match state
        .get_entity(EntityType::SchoolOfThought, &name)
        .await
        .map_err(|e| e.to_string())?
    {
...
#[tauri::command]
pub async fn create_school_of_thought(
    state: State<'_, EncyclopediaDb>,
    vault: State<'_, VaultManager>,
    request: CreateSchoolOfThoughtRequest,
) -> Result<String, String> {
    handle_create(state, vault, request).await
}

#[tauri::command]
pub async fn update_school_of_thought(
    state: State<'_, EncyclopediaDb>,
    vault: State<'_, VaultManager>,
    name: String,
    request: CreateSchoolOfThoughtRequest,
