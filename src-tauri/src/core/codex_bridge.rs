...
async fn load_latest_codex_history(app: AppHandle, _working_dir: PathBuf) -> Result<(), String> {
    ...
    let project_name = _working_dir
        ...
        .to_lowercase();
    println!(
        "[codex_bridge] >>> Scoping history search to project family: {}",
        project_name
    );
    ...
    fn find_latest_in_dir(
        ...
        latest_time: &mut std::time::SystemTime,
    ) {
    ...
    if let Some(path) = latest_file {
        println!(
            "[codex_bridge] >>> AUTO-RECOVERY: Loading latest session from: {:?}",
            path
        );
...
pub async fn send_codex_input(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let mut child = match child {
            Ok(c) => {
                eprintln!(
                    "[codex_bridge] !!! SUCCESS: Child spawned with PID: {:?}",
                    c.id()
                );
                c
            }
        ...
        eprintln!(
            "[codex_bridge] --- Process exited with status: {:?}",
            status
        );
    });
...
#[tauri::command]
pub async fn stop_codex_session(state: tauri::State<'_, CodexState>) -> Result<(), String> {
...
#[tauri::command]
pub async fn get_codex_session_id(state: tauri::State<'_, CodexState>) -> Result<String, String> {
