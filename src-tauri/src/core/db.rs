...
impl EncyclopediaDb {
    ...
    pub async fn get_entity(
        ...
        let row: Option<(String,)> =
            sqlx::query_as("SELECT data FROM entities WHERE entity_type = $1 AND name = $2")
                .bind(entity_type.to_string())
                .bind(name)
    ...
    pub async fn get_entity_file_path(
        &self,
        entity_type: EntityType,
        name: &str,
    ) -> Result<Option<String>, sqlx::Error> {
        let row: Option<(String,)> =
            sqlx::query_as("SELECT file_path FROM entities WHERE entity_type = $1 AND name = $2")
                .bind(entity_type.to_string())
                .bind(name)
                .fetch_optional(&self.pool)
                .await?;
    ...
    pub async fn delete_entity_by_file_path(&self, file_path: &str) -> Result<u64, sqlx::Error> {
        let row: Option<(String, String)> =
            sqlx::query_as("SELECT entity_type, name FROM entities WHERE file_path = $1")
                .bind(file_path)
                .fetch_optional(&self.pool)
                .await?;
    ...
    pub async fn clear_outgoing_relations(
        &self,
        entity_type: EntityType,
        name: &str,
    ) -> Result<u64, sqlx::Error> {
