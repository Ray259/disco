...
impl VaultManager {
    ...
    pub async fn full_sync(&self, db: &EncyclopediaDb) -> Result<SyncReport, String> {
        ...
        // Optimization: Wait for async directory traversal to complete before processing files
...
/// Iteratively gets all absolute file paths in a directory using async I/O.
/// Optimization: Uses a stack-based approach with tokio::fs::read_dir to prevent blocking
/// the async executor and avoid recursive memory overhead.
